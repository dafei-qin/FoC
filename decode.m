function info = decode(code, k)
    global infoLen logRadix nSeg p;
    code1x = binaryToRadix(code(1, 1: nSeg * logRadix));
    code2x = binaryToRadix(code(1, nSeg * logRadix + 2: 2 * nSeg * logRadix + 1));
    [~, code1y] = findY(code1x);
    if mod(code1y(1), 2) ~= code(nSeg * logRadix + 1)
        code1y = minusBig(p, code1y);
    end
    [~, code2y] = findY(code2x);
    if mod(code2y(1), 2) ~= code(2 * nSeg * logRadix + 2)
        code2y = minusBig(p, code2y);
    end
    [mx, ~] = decodeP(code1x, code1y, code2x, code2y, k);
    info = zeros(1, infoLen * logRadix);
    for i = 1: infoLen
        for j = 1: logRadix
            info((i - 1) * logRadix + j) = bitand(1, bitshift(mx(i), -(j - 1)));
        end
    end
end
